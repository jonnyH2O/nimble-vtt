    ...
    <div className="h-screen bg-gray-900 text-white flex flex-col">
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded text-sm ${showGrid ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-600 hover:bg-gray-500'
                          }`}
                      >
                        {showGrid ? 'ON' : 'OFF'}
                      </button>
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded text-sm ${darknessMode ? 'bg-purple-600 hover:bg-purple-700' : 'bg-gray-600 hover:bg-gray-500'
                          }`}
                      >
                        {darknessMode ? 'ON' : 'OFF'}
                      </button>
          ...
          {[...rollingDice.map(r => ({ ...r, type: 'rolling' })), ...diceRolls.map(r => ({ ...r, type: 'result' }))]
            ...
            .map((item) =>
              item.type === 'rolling' ? (
                ...
                <div
                  key={`result-${item.id}`}
                  className={`bg-green-600 text-white px-6 py-4 rounded-lg shadow-2xl border-4 border-green-400 transition-all duration-300 ${item.fading ? 'dice-fade' : 'dice-pop'
                    }`}
                >
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 p-4 overflow-hidden">
          <div
            ref={boardRef}
            className={`relative bg-gray-700 rounded w-full h-full min-h-[600px] overflow-hidden ${drawMode === 'select' ? 'cursor-grab active:cursor-grabbing' : ''
              }`}
                        ...
                        <div className="relative">
                          {token.image ? (
                            ...
                            <div className="relative">
                              ...
                              >
                                <div className={`w-full h-full rounded-full flex items-center justify-center ${getTokenBgColor(token.type)}`}>
          ...
          <div className="bg-gray-700 border-b border-gray-600 p-3 flex items-center justify-between">
            <div className="flex gap-2">
              <button
                ...
                className={`p-2 rounded flex items-center justify-center ${sidebarView === 'turnOrder' ? 'bg-blue-600' : 'bg-gray-600 hover:bg-gray-500'
                  }`}
                title="Turn Order"
              >
                <List size={20} />
              ...
              <button
                ...
                className={`p-2 rounded flex items-center justify-center ${sidebarView === 'dictionary' ? 'bg-blue-600' : 'bg-gray-600 hover:bg-gray-500'
                  }`}
            ...
            {sidebarView === 'turnOrder' && (
              <button
                ...
                className={`text-sm px-3 py-1.5 rounded flex items-center gap-1 ${deleteMode ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-600 hover:bg-gray-500'
                  }`}
              >
                <Trash2 size={14} />
                    ...
                    return (
                      <div key={`${item.id}-${index}`}>
                        ...
                        <div
                          ...
                          className={`bg-gray-700 p-3 rounded ${deleteMode && !isLegendaryEcho ? 'cursor-pointer hover:bg-red-900' :
                            !isLegendaryEcho && !expandedConditions[`${item.id}-${isLegendaryEcho ? 'echo-' + index : 'main'}`] ? 'cursor-move' : ''
                            } ${selectedToken === item.id && !isLegendaryEcho ? 'ring-2 ring-orange-500' : ''} ${isLegendaryEcho ? 'opacity-75 ml-4' : ''
                              ...
                              {isLegendaryEcho && (
                                <div className="flex gap-1">
                                  <button
                                    ...
                                    className={`flex-1 h-8 rounded transition-colors ${token.actions[getDisplayTurnOrder()
                                      .slice(0, index)
                                      .filter(i => i.id === item.id && i.isLegendaryEcho).length]
                                      ? 'bg-gray-500'
